/**
 * HoneyMoonTrust Crawler - honeymoontrust.co.uk
 * Crawl truyện tranh NetTruyen mirror tiếng Việt
 */
...
const BASE_URL = 'https://honeymoontrust.co.uk'
...
async function getStoryList(page = 1) {
    ...
    $('article, .manga-item, .story-item, .item').each((_, el) => {
        const $el = $(el)
        const $link = $el.find('a[href*="truyen-tranh"]').first()
        const title = $el.find('h3, .title, .story-name').text().trim() || $link.attr('title')
...
async function getStoryDetails(storyUrl) {
    ...
    const title = $('h1, .manga-title, .story-title').first().text().trim()
    const author = $('.author a, .tacgia a, span:contains("Tác giả") + a').text().trim() || 'Unknown'
    const description = $('.manga-content, .story-description, .description').text().trim()
    const cover = $('.manga-info img, .story-cover img, article img').first().attr('src')
    ...
    // Get chapter list
    const chapters = []
    $('a[href*="chapter"], .chapter-list a, .list-chap a').each((_, el) => {
        const $el = $(el)
...
async function getChapterContent(chapterUrl) {
    ...
    const images = []
    $('.reading-detail img, .chapter-content img, .page-chapter img, article img').each((_, el) => {
    ...
    const textContent = $('.chapter-content, .content-chapter, article')
        .clone()
...
function saveStory(story) {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(story.title, story.slug, story.author, story.cover, story.description, 'honeymoontrust', story.sourceUrl)
...
async function crawl(pages = 1, chaptersPerStory = 5) {
    console.log('🚀 Starting HoneyMoonTrust crawler...')
